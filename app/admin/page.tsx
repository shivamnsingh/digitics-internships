...
export default async function Admin() {
  ...
  return (<div className="mx-auto max-w-7xl space-y-8 px-5 py-8"><header className="flex items-center justify-between"><div className="flex items-center gap-4"><Logo h={34} /><h1 className="display text-3xl font-extrabold">Admin</h1></div><Logout /></header>
    {failed > 0 && <div className="space-y-3"><p role="alert" className="rounded-xl border border-red-400/50 bg-red-400/10 p-4 text-sm">{failed} application(s) are saved here but failed to sync to the spreadsheet. Check the sync configuration; the data is safe in the database.</p><RetryFailed count={failed} /></div>}
